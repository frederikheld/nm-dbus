...
class Connections extends DBusObject {
    ...
    /**
     * Returns a list of connections which have a `connectionId`
     * matching the given `pattern`. `pattern` is directly passed to
     * String.prototype.match(), so you can pass any string that can
     * be turned into a valid RegExp (see [1] for details).
     * 
     * Make sure that any string you would like to match is properly escaped
     * if it contains (or could contain) characters that have a special
     * meaning in a RegEx! E.g. parentheses, which are quite common in connection
     * names. You can use utils.strings.escapeRegExString() to mask this part of the pattern.
     * This does not mean that you need to escape the whole pattern. This only
     * applies to the parts of the pattern that you don't want to act as a RegEx.
     * 
     ...
     */
    async getById (pattern) {
        const allConnections = await this.getAll()

        const matches = allConnections.filter((connectionObject) => {
            return connectionObject.connectionId.match(pattern)
        })
