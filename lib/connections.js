...
class Connections extends DBusObject {
    ...
    async createWifi (ssid, psk) {
        ...
        const dbusSettings = {
            ...
            '802-11-wireless': {
                mode: new dbus.Variant('s', 'infrastructure') // Note: for creating an AccessPoint, the mode will be 'ap'
            },
        ...
        dbusSettings['802-11-wireless'].ssid = new dbus.Variant('ay', Buffer.from(ssid))
        dbusSettings['802-11-wireless-security'].psk = new dbus.Variant('s', psk)
    ...
    async getAll () {
        ...
        // get all connections:
        const dbusSettingsPath = this.dbusConfig.basePath + '/Settings'
        const proxyObject = await this.bus.getProxyObject(this.dbusConfig.baseName, dbusSettingsPath)
        const managerInterface = proxyObject.getInterface(this.dbusConfig.baseName + '.Settings')
